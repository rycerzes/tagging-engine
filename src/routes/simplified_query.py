...
async def get_simplified_query_data(
    ...
    try:
            ...
            try:
                with open(cache_file, "r") as f:
                    ...
                    if result.get("video_id") == video_id:
                        ...
                        break
                    # Check if this video_id is a cached version
                    cache_info = result.get("cache_info", {})
                    if cache_info.get("original_video_id") == video_id:
                        cached_data = cache_content
                        break
            except Exception:
                continue
        ...
        # Determine if this is cached and get the correct video_id for queries
        cache_info = result.get("cache_info", {})
        is_cached = cache_info.get("cached", False)
        original_video_id = cache_info.get("original_video_id") if is_cached else video_id
        query_video_id = original_video_id if is_cached else video_id
        ...
        # Get fashion matches from query service
        ...
        for crop in crops_data:
            ...
            high_confidence_matches = [match for match in fashion_matches if match["score"] >= 0.75]
        ...
        return CombinedVideoResponse(
            ...
            file_hash=file_hash,
            cached=is_cached,
            cached_from_video_id=original_video_id if is_cached else None,
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to get simplified query data: {str(e)}"
        )
